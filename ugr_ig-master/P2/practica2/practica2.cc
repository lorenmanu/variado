...
scene escena;
figura_revolucion base,cima,peon;
...
void draw_objects() {
    if(copia_argv == 1){
        base.draw(escena.getModo());
        cima.draw(escena.getModo());
    }else{
        peon.draw(escena.getModo());
    }
   }
...
int main(int argc, char **argv) {
    copia_argv = atoi(argv[1]);
    cima = figura_revolucion("models/cima_torre.ply",true,false,true);
    base = figura_revolucion("models/base_torre.ply",false,true,false);
    peon = figura_revolucion("models/peon.ply",true,true,false);
