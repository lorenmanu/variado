...
void draw_objects() {
    if(copia_argc == 3)
    	f.draw(escena.getModo());
    else
	c.draw(escena.getModo());
}
...
int main(int argc, char **argv) {
    copia_argc = argc;
    if(argc < 3){
	std::cout << "Dibujaré un cubo" << std::endl;
	escena = scene(UI_window_pos_x, UI_window_pos_y, UI_window_width, UI_window_height,0);
    }else{
        f = figura_ply(argv[1]);
        escena = scene(UI_window_pos_x, UI_window_pos_y, UI_window_width, UI_window_height,atoi(argv[2]));
    }
